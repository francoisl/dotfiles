function _tide_item_git
    if git branch --show-current 2>/dev/null | string shorten -"$tide_git_truncation_strategy"m$tide_git_truncation_length | read -l location
        ...
        # Don't show branch name for 'main'
        if [ "main" = $location ]; set location ''; else; set location ' '$location; end
    ...
    set -l stat (git $_set_dir_opt --no-optional-locks status --porcelain | grep -vE '^\?\?.*\.diff$' 2>/dev/null)
...
(0|(?<dirty>.*))\n(0|(?<untracked>.*))(\n(0|(?<behind>.*))\t(0|(?<ahead>.*)))?' \
    ...
    _tide_print_item git $_tide_location_color$tide_git_icon " " (set_color white; echo -ns $location
        ...
        set_color $tide_git_color_untracked; echo -ns ' 󱙝 '$untracked)
end
