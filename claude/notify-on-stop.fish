#!/usr/bin/env fish
# Use `terminal-notifier` to show a notification when Claude finishes, if wezterm is not active, or if another tab/pane is active
...
if test "$frontmost" != "wezterm-gui"
    ...
    terminal-notifier -message "Claude is done" -title "Claude Code" -sound Bottle -sender com.github.wez.wezterm
else if test "$WEZTERM_PANE" != "$focused_pane"
    # WezTerm active but different pane focused - no sender to avoid suppression
    terminal-notifier -message "Claude is done" -title "Claude Code"
end
